...
class DatabaseConfig:
    def __init__(self, app_config):
        ...
        self.model_class: Type[BronzeRenconEntity] = BronzeRenconEntity
        self.mode: ModePersistence = ModePersistence.UPDATE
        self.conflict_cols: tuple[str, ...] = ('id_sec', )
        self.update_cols: tuple[str, ...] = ('codcomp', 'codcta', 'importe', 'codasi', 'fecasi', 'origen')
...
class PipelineConfig:
    def __init__(self, app_config):
        self.pipeline_name: str = "Bronze Rencon Entity Pipeline"
        self.mode_pipeline: RunMode = app_config.run_mode

        self.SOURCE_BY_MODE: Dict[RunMode, List[SourceSpec]] = {
            RunMode.INICIAL: [
                SourceSpec("fenix", "FENIX", "initialization", "rencon.sql")
            ],
            RunMode.INCREMENTAL: [
                SourceSpec("fenix", "FENIX", "incremental", "rencon_incremental.sql")
            ]
        }


class BronzeRenconPipeline:
    def __init__(
            self,
    ...
    def _build_pipeline(self, spec: SourceSpec, sql_text: str, params: Optional[Dict[str, object]]) -> LoggingPipeline:
        steps = [
            ...
            ("Load DataWarehouse Bronze Rencon", DWBatchedLoader(
