...
from dwh_facturacion.entities.bronze.bronze_grupo_vendedor_entity import BronzeGrupoVendedorEntity
...
class DatabaseConfig:
    def __init__(self, app_config):
        ...
        self.model_class: Type[BronzeGrupoVendedorEntity] = BronzeGrupoVendedorEntity
        self.mode: ModePersistence = ModePersistence.UPDATE
        self.conflict_cols: tuple[str, ...] = ('factura_externa', )
        self.update_cols: tuple[str, ...] = ('update_date', 'username', 'grupo_vendedor', 'fecha_registro')
...
class PipelineConfig:
    def __init__(self, app_config):
        self.pipeline_name: str = "Bronze Grupo Vendedor Entity Pipeline"
...
class BronzeGrupoOperadorPipeline:
    def __init__(
            self,
    ...
    def _build_pipeline(self, spec: SourceSpec, sql_text: str, params: Optional[Dict[str, object]]) -> LoggingPipeline:
        steps = [
            ("Extract Data from CAMUNDA Database",
             DatabaseExtractor(db_alias=spec.db_alias_load, query=sql_text, params=params)),
            ("Load DatawareHouse Bronze Grupo Operador", DWBatchedLoader(
