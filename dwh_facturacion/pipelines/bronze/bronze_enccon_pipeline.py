...
class DatabaseConfig:
    def __init__(self, app_config):
        ...
        self.model_class: Type[BronzeEnconEntity] = BronzeEnconEntity
        self.mode: ModePersistence = ModePersistence.UPDATE
        self.conflict_cols: tuple[str, ...] = ('id_codasi', )
        self.update_cols: tuple[str, ...] = ('codcomp', 'codasi', 'fecasi', 'fecha')
...
class PipelineConfig:
    def __init__(self, app_config):
        self.pipeline_name: str = "Bronze Enccon Entity Pipeline"
        self.mode_pipeline: RunMode = app_config.run_mode

        self.SOURCE_BY_MODE: Dict[RunMode, List[SourceSpec]] = {
            RunMode.INICIAL: [
                SourceSpec("fenix", "FENIX", "initialization", "enccon.sql")
            ],
            RunMode.INCREMENTAL: [
                SourceSpec("fenix", "FENIX", "incremental", "enccon_incremental.sql")
            ]
        }


class BronzeEnconPipeline:
    def __init__(
            self,
    ...
    def _build_pipeline(self, spec: SourceSpec, sql_text: str, params: Optional[Dict[str, object]]) -> LoggingPipeline:
        steps = [
            ...
            ("Load DataWarehouse Bronze Enccon", DWBatchedLoader(
